...
sub generate_upgrade_script {
    ...
    if ($new_minor != $old_minor) {
        ...
        if ($old_mayor == 2 or $old_minor < 6) {
            ...
            push @commands, drop_special_case_function("pgr_withpointsdd(text,text,anyarray,double precision,character,boolean,boolean,boolean)");
            push @commands, drop_special_case_function("pgr_withpointsdd(text,text,bigint,double precision,character,boolean,boolean)");
